...
class PluginTopicsoundche_ActionTopicsoundche extends ActionPlugin {
...
/**********************************************************************************
	...
	protected function EventEdit() {
		...
		if (isset($_REQUEST['submit_topic_publish']) or isset($_REQUEST['submit_topic_save'])) {
			...
			$_REQUEST['topic_audio_hidden']=$oTopic->getAudio();
			$_REQUEST['topic_text']=$oTopic->getTextSource();
			$_REQUEST['topic_sponsor']=$oTopic->getSponsors();
			$_REQUEST['topic_audio_text_version']=$oTopic->getTextversionSource();
	...
	protected function EventAdd() {
		 ...
		 * Загружаем переменные в шаблон
		 */
		$this->Viewer_Assign('title_podcast',Config::Get('plugin.topicsoundche.title_podcast'));
		$this->Viewer_Assign('aBlogsAllow',$this->Blog_GetBlogsAllowByUser($this->oUserCurrent));
	...
	protected function SubmitAdd() {
		...
		$oTopic->setSponsors(getRequest('topic_sponsor'));
		$oTopic->setTextversionSource(getRequest('topic_audio_text_version'));
		$oTopic->setTextversion(getRequest('topic_audio_text_version'));
		$oTopic->setTags(getRequest('topic_tags'));
		$oTopic->setUserId($this->oUserCurrent->getId());
		$oTopic->setType('soundche');
	...
	protected function SubmitEdit($oTopic) {
		//fix переносы строк при сохранении спонсоров
		$topic_sponsor = $this->Text_Parser(getRequest('topic_sponsor'));
		$topic_audio_text_version = $this->Text_Parser(getRequest('topic_audio_text_version'));
		...
		$oTopic->setTitle(strip_tags(getRequest('topic_title')));
		$oTopic->setTextSource(getRequest('topic_text'));
		$oTopic->setSponsors($topic_sponsor);
		$oTopic->setTextversionSource(getRequest('topic_audio_text_version'));
		$oTopic->setTextversion($topic_audio_text_version);
		$oTopic->setTags(getRequest('topic_tags'));
		$oTopic->setUserIp(func_getIp());
		/**
		 * Проверка корректности полей формы
		 ...
		 */
		//Определяем есть ли новый файл
		if (isset($_FILES['topic_audio']) and is_uploaded_file($_FILES['topic_audio']['tmp_name'])) {
			if ($sAudioPath=$this->PluginTopicsoundche_Topicsoundche_UploadAudioBySubmit($oTopic,$_FILES['topic_audio'])) {
				$oTopic->setAudio($sAudioPath);
			}
		}
    ...
    protected function EventAdmin() {
    	if (isPost('submit_config')) {
    		$this->Security_ValidateSendForm();
    		Config::Set('plugin.topicsoundche.myvar', $_POST['topic_name']);
    	}
    	$config = Config::Get('plugin.topicsoundche');
